...
## 2026-09-13 - Lazy Loading Off-Screen Images

**Learning:** Unmeasurable micro-optimizations (like hoisting small lookups) are not effective for actual performance improvements. Off-screen images without `loading="lazy"` are a universally recognized, measurable frontend bottleneck causing unnecessary early network requests and slower page load times.
**Action:** Add `loading="lazy"` to off-screen `<img />` tags (like in galleries, blog post covers, and testimonials below the fold) to defer network requests and tangibly improve page load performance.
