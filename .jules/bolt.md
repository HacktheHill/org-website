...
## 2024-03-22 - Avoid loading="lazy" for components with intentional preloading
**Learning:** The site intentionally preloads images so that scroll animations, AOS fade-ins, and continuous marquees are always rendered with their assets present. Adding `loading="lazy"` to galleries, animated testimonials, or `overflow:hidden` carousels causes visible artifacts (e.g. images popping in mid-animation or staying blank during source swaps).
**Action:** Do not apply `loading="lazy"` blindly to off-screen images in this codebase. Preloading is prioritized for UX over reducing initial network footprint for these specific animated sections.
